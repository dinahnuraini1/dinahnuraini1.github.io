...
if (isset($_GET['id'])) {
    ...
    $query_select = "SELECT gambar FROM home WHERE id = '$id'";
    ...
    if (mysqli_num_rows($result_select) > 0) {
        ...
        // Query untuk menghapus data home dari database
        $query_delete = "DELETE FROM home WHERE id = '$id'";
        $result_delete = mysqli_query($conn, $query_delete);

        if ($result_delete) {
            // Hapus gambar dari server jika data berhasil dihapus dari database
            if ($gambar) {
                $file_path = '../dokumentasi_home/' . $gambar;
