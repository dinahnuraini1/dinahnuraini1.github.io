...
if (isset($_GET['id'])) {
    ...
    $query_select = "SELECT gambar FROM pantauan WHERE id = '$id'";
    ...
    if (mysqli_num_rows($result_select) > 0) {
        ...
        // Query untuk menghapus data pantauan dari database
        $query_delete = "DELETE FROM pantauan WHERE id = '$id'";
        $result_delete = mysqli_query($conn, $query_delete);

        if ($result_delete) {
            // Hapus gambar dari server jika data berhasil dihapus dari database
            if ($gambar) {
                $file_path = '../dokumentasi_pantauan/' . $gambar;
