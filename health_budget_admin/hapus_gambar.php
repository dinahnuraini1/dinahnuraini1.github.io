...
if (isset($_GET['id'])) {
    ...
    $sql_select = "SELECT gambar FROM anggaran_perkegiatan WHERE id = '$gambar_id'";
    ...
    if ($result->num_rows > 0) {
        ...
        $sql_delete = "DELETE FROM anggaran_perkegiatan WHERE id = '$gambar_id'";

        if ($conn->query($sql_delete) === TRUE) {
            // Hapus file gambar dari penyimpanan (jika perlu)
            $file_path = "gambar_perkegiatan/" . $gambar_file;
            ...
            echo "<script>window.location.href = 'anggaran_perkegiatan.php';</script>";
            exit;
        } else {
